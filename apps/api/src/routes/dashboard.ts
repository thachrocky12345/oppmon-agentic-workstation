...
dashboardRouter.get('/overview', requireRole('TENANT_ADMIN'), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const agents = await query(`
    SELECT a.id, a.name, a.config, a.created_at, a.tenant_id, a.status, a.last_seen,
      (SELECT MAX(e.timestamp) FROM events e WHERE e.agent_id = a.id) as last_active,
      (SELECT COUNT(*) FROM events e WHERE e.agent_id = a.id AND e.timestamp > NOW() - INTERVAL '24 hours') as events_24h,
      (SELECT COUNT(*) FROM events e WHERE e.agent_id = a.id AND e.timestamp > NOW() - INTERVAL '7 days') as events_7d,
 ...
 *
 * NOTE: Events table doesn't have tenant_id directly - filter through agents relation.
 */
 ...
 *
 * NOTE: Computes trends directly from events table.
 */
dashboardRouter.get('/trends', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const result = await query(`
    ...
    WHERE a.tenant_id = $1 AND e.timestamp > NOW() - ($2::int || ' days')::interval
...
dashboardRouter.get('/anomalies', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const result = await query(`
    ...
    FROM events e
    JOIN agents a ON a.id = e.agent_id
    WHERE a.tenant_id = $1 AND e.severity = 'error'
    ORDER BY e.timestamp DESC
    LIMIT $2
  `, [req.tenantId, limit]);
...
dashboardRouter.get('/agent/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const statsResult = await query(`
    SELECT DATE(e.timestamp) as day,
      ...
      COUNT(CASE WHEN e.severity = 'error' THEN 1 END) as errors
    FROM events e
    WHERE e.agent_id = $1 AND e.timestamp > NOW() - INTERVAL '30 days'
    GROUP BY DATE(e.timestamp)
    ORDER BY day ASC
  `, [req.params.id]);
