...
调用工具时，请按照以下格式:
```
你的思考过程...<|action_start|><|plugin|>{{"name": "tool_name", "parameters": {{"query": "value1"}}}}<|action_end|>
```
...
```
Your thought process...<|action_start|><|plugin|>{{"name": "tool_name", "parameters": {{"query": "value1"}}}}<|action_end|>
```
