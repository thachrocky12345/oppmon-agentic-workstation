...
from .lagent.agents import AgentReturn, BaseAgent, Internlm2Agent,Internlm2Agent, Internlm2Protocol
...
class SearcherAgent(Internlm2Agent):
    ...
    def stream_chat(self,
        ...
        logging.debug(f'Current query: {message}')
...
class MindSearchAgent(BaseAgent):
    ...
    def _generate_reference(self, agent_return, code, as_dict):
        ...
        for node_name in node_list:
            ...
            if actions:
                if as_dict and not actions[0]['result']:
                    logging.warning(f"Actions are empty or have no valid results. {actions}")
                    continue

                    # Ensure result[0] exists and contains 'content'
                if not as_dict and not actions[0].result:
                    logging.warning(f"Result is invalid or does not contain 'content'. {actions}")
                    continue
    ...
    def execute_code(self, command: str, return_early=False):
        ...
        def run_command(cmd):
            try:
                ...
                logging.debug(f"Local dict keys: {list(self.local_dict.keys())}")
