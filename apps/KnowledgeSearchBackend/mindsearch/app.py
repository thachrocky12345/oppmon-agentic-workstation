...
from mindsearch.agent.lagent.schema import AgentStatusCode
from pydantic import BaseModel
from sse_starlette.sse import EventSourceResponse

from mindsearch.agent import init_agent

# v2 agent (additive — leaves legacy /solve untouched during cutover).
try:
    from mindsearch.agent_v2.app import mount_v2 as _mount_v2
except Exception as _v2_err:  # noqa: BLE001
    _mount_v2 = None
    _V2_IMPORT_ERROR = _v2_err
else:
...
def setup_logging(debug: bool = False):
    ...
    debug = debug or os.getenv('MINDSEARCH_DEBUG', '').lower() in ('1', 'true', 'yes')
    ...
    if debug:
        ...
        log_level = logging.ERROR  # Only ERROR and CRITICAL
...
app = FastAPI(docs_url='/', timeout=30)  # Add global timeout
...
# Mount the v2 reactive agent at /solve_v2 (additive).
if _mount_v2 is not None:
    _mount_v2(app)
elif _V2_IMPORT_ERROR is not None:
    logging.warning("agent_v2 not mounted: %s", _V2_IMPORT_ERROR)
...
async def run(request: GenerationParams):
    ...
    # agent = init_agent(lang='en', model_format='gpt4', search_engine='BingSearch')
...
if __name__ == '__main__':
    ...
    # Set uvicorn log level based on our debug setting
    uvicorn_log_level = 'debug' if args.debug or os.getenv('MINDSEARCH_DEBUG') else 'error'
    
    print(f"🚀 Starting MindSearch API server...")
    ...
    uvicorn.run(app, host='0.0.0.0', port=8002, log_level=uvicorn_log_level)
