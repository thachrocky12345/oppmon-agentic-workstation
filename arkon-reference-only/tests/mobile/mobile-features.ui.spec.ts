...
test.describe("Mobile Dashboard", () => {
  ...
  test("mobile dashboard renders health content @regression @mobile", async ({ page }) => {
  ...
  test("mobile bottom nav is visible @regression @mobile", async ({ page }) => {
    await page.goto(`${MC_URL}/`);
    await page.waitForLoadState("domcontentloaded");
  ...
  test("mobile stat tiles render in 2-column grid @regression @mobile", async ({ page }) => {
    await page.goto(`${MC_URL}/`);
    await page.waitForLoadState("domcontentloaded");
  ...
  test("no console errors on mobile dashboard @regression @mobile", async ({ page }) => {
    const errors: string[] = [];
  ...
  test("mobile More sheet opens on tap @regression @mobile", async ({ page }) => {
    await page.goto(`${MC_URL}/`);
    await page.waitForLoadState("domcontentloaded");
...
test.describe("PWA Manifest", () => {
  test("manifest.json is accessible @regression @mobile", async ({ request }) => {
  ...
  test("service worker is registered @regression @mobile", async ({ page, context }) => {
    await context.request.post(`${MC_URL}/api/auth/init`, {
      headers: { Authorization: `Bearer ${ADMIN_TOKEN}` },
    });
    await page.goto(`${MC_URL}/`);
